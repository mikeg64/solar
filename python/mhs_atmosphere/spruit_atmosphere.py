...
for l_string in ('l_linear','l_const','l_sqrt','l_square'):
    option_pars['l_const'], option_pars['l_sqrt'], option_pars['l_square'],\
                       option_pars['l_linear'] = False,False,False,False 
    option_pars[l_string] = True
...
# may be worthwhile locating on /data if files are large
    ...
    if rho.min()<0 or pressure.min()<0:
        print"FAIL rank {}: negative rho.min() {} and/or pressure.min() {}."\
                        .format(rank,rho.min(),pressure.min())
