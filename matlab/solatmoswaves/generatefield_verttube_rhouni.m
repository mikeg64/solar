function [simparams, simgridinfo, simdata]=generatefield_verttube_rhouni(simparams, simgridinfo, simdata, mode)
...
if strcmp(mode,'vertfieldtube')
    ...
    ymax=-(simparams.domain_left_edge(2)-simparams.domain_right_edge(2));
    zmax=-(simparams.domain_left_edge(3)-simparams.domain_right_edge(3));
    
    bx=zeros(nx1,nx2,nx3);
    by=zeros(nx1,nx2,nx3);
    bz=zeros(nx1,nx2,nx3);
    
    rho=zeros(nx1,nx2,nx3);
...
A=R2/2;
    ...
    for j=1:nx2
        y(j)=(simparams.domain_left_edge(2)+dy*(j-1))-(ymax/2);
    end
    for k=1:nx3
        z(k)=(simparams.domain_left_edge(3)+dz*(k-1))-(zmax/2);
    end
...
bx(i,j,k)=bx(i,j,k);
by(i,j,k)=by(i,j,k);
rho(i,j,k)=rho(i,j,k)+rho0zz.*xf(i,j,k);
...
rho1=simdata.w(:,:,:,1)+rho+simdata.w(:,:,:,10);
p=Bvari+(simdata.w(:,:,:,5)+simdata.w(:,:,:,9))*(gamma-1.0);
